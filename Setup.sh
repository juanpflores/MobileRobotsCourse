...
if [ $# -eq 0 ] ; then
	...
	echo -e "\t-i, --install"
	echo -e "\t\t To install Mobile-robots-course software for first time"
	echo -e "\t\t (${FRM}${RED}${BGBLACK}must be executed as sudo${NC})"
	echo -e "\t-u, --update"
	echo -e "\t\t To update an already existent installation,"
	...
	if [ "$1" == "-i" ] || [ "$1" == "--install" ]; then
		...
		#SCRIPT START
		#THE REAL STUFF
		echo -e "${FRM}${WHITE}${BGBLUE} Installing dependencies ${NC}"
		...
		echo -e "${FRM}${GREEN}${BGBLUE} Dependencies have been installed ${NC}"
		...
		#echo "alias jsea='roslaunch surge_et_ambula justina.launch'" >> /home/$USER/.bashrc
		#echo "alias jseas='roslaunch surge_et_ambula justina_simul.launch'" >> /home/$USER/.bashrc
		echo -e "${FRM}${WHITE}${BGBLUE}Copying the rules of USB ports to system${NC}"
		cd $SOURCE_DIR
		sudo cp ToInstall/USB/80-justinaRobot.rules /etc/udev/rules.d/
		sudo udevadm control --reload-rules && sudo service udev restart && sudo udevadm trigger
		serialsPort=($(ls /dev/ttyACM*))
		...
		echo -e "${FRM}${RED}${BGWHITE}You can now ${NC}${FRM}${BLACK}${BGWHITE}behold${NC}${FRM}${RED}${BGWHITE} the power of MobileRobots FI-UNAM software${NC}"
	else
