...
void callback_start_move(const sensor_msgs::LaserScan::ConstPtr& msg)
{
	float front_dist = msg->ranges[msg->ranges.size() / 2];
	std::cout << "Front distance = " << font_dist << std::endl;
...
int main(int argc, char**argv)
{
	...
	ros::NodeHandle n;
	ros::Suscriber sub_start = n.suscribe("/start_move", 1, callback_start_move);

	ros::Suscriber sub_scan = n.suscribe("hasrdware/scan", 1, callback_scan);
...
while(ros::ok())
{
	...
	if(move && !obstacle)
		pub_cdm_vel.publish(msg_cmd_vel)
	ros::spinOnce();
	loop.sleep();
}
