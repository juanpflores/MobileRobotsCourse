...
{   
    ...
    //Use inflate map method
    inflatedMap = PathSmoother::InflateObstacles(originalMap, inflation_radius);
    pubInflMap.publish(inflatedMap); //publish it
    std::cout << "Practica04.->Getting nearness map with radius " << nearness_radius << std::endl;
    //Get nearness map for inflated map, taking into account the nearness radius (radius of influence)
    ...
    //Choose from each of the path planning algorithms
    switch(method)
    {
        case METHOD_BFS:
            std::cout << "Practica04.->Planning path by BREADTH FIRST SEARCH algorithm for:" << std::endl;
            //If the Algorithm doesn't return a path clear messages of both original and smoothed paths
            // and return false
    ...
    //Publish original path if path planning didn't fail
    pubOriPath.publish(msgOriginalPath);

    std::cout << "Practica04.->Smoothing path with parameters alpha="<<smoothing_alpha<<" and beta="<<smoothing_beta << std::endl;
    //execute path smoothing with gradient descent, pass alpha (weight of the original path) and beta(weight of the smoothed path)
