...
int main(int argc, char** argv)
{
    ...
    while(gradient_magnitude > tolerance && epochs < max_epochs)
      {
	for(int i = 0; i < training_images.size(); i++)
	  {
	    estimated_y = p.evaluate((float*)training_images[i].data);
	    float desired_output = training_labels[i] == digit_to_train ? 1.0 : 0.0;
	    
	    //Sólo es necesario calcularlo una vez:
	    float factor = (estimated_y - desired_output)*(estimated_y*(1 - estimated_y));
	    
	    for(int j = 0; j < p.w.size(); j++)
	      {
		gradient_i = factor * ((float*)training_images[i].data)[j];
		p.w[j] -= delta * gradient_i;
		gradient_magnitude += gradient_i * ((float*)training_images[i].data)[j];
	      }

	    gradient_t = factor;
	    p.theta -= delta * gradient_t; 
	  }
