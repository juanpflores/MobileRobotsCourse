...
int weighted_sample_index(std::vector<float>& weights, float max_weight)
{
     ...
     */
     ...
     float random = rnd.uniformReal(0, max_weight);
...
int main(int argc, char** argv)
{
     ...
     */
     for(int i = 0; i < robots.size(); i++){
	/*
         * rnd.uniformReal(lower bound, upper bound)
         * For x and y, lower and upper bound are the map's dimentions
         * For a, lower and upper bound are in (-PI,PI]
         */
	robots[i].x = rnd.uniformReal(0, 10);
	robots[i].y = rnd.uniformReal(0, 8);
	robots[i].a = rnd.uniformReal(-PI, PI);
     }
	...
	error_x = last_robot_x - robot_x;
	error_y = last_robot_y - robot_y;
	distance = sqrt( (pow(error_y,2) + pow(error_x,2)) );
	angle = abs(last_robot_a - robot_a);
	if(distance > DIST_THRESHOLD || angle > ANGLE_THRESHOLD){
		for(int i=0; i < robots.size(); i++){
		...
		std::vector<float> weights;
    		weights.resize(particle_measurements.size());
		weights = measurement_weights(particle_measurements, real_scan);
