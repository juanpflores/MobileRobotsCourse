...
float Perceptron::evaluate(float* data)
{
    ...
    sigmoid = (1.0/(1.0 + exp(-output + theta)));
    output = sigmoid;
    return output;
}
...
int main(int argc, char** argv)
{
    ...
    //Store desired outputs for each train data
    for(int i = 0; i < training_labels.size(); ++i){
        desired_output[i] = training_labels[i] == digit_to_train ? 1.0 : 0.0;
    }
    do{ 
        gradient_magnitud = 0;
        for(int i = 0; i < training_images.size(); ++i){
            estimated_y = p.evaluate((float*)training_images[i].data);
            gradient = 0;
            for(int j = 0; j < training_images.size(); ++j){
                gradient += (estimated_y - desired_output[j])*(estimated_y*(1-estimated_y));
            }
            for(int j = 0; j < p.w.size(); j++){
                p.w[j] -= delta * gradient * (float)training_images[i].data[j];
                gradient_magnitud+=gradient * (float)training_images[i].data[j];
            }
            p.theta -= delta * gradient;
        }
        epoch++;         
      }while(abs(gradient_magnitud) > tolerance && epoch < max_epochs);

    std::cout<< "Training has end" << std::endl;
