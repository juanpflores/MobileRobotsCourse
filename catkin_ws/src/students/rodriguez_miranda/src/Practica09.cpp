...
int main(int argc, char** argv)
{
    ...
    float epochs=0;
    float epsi_i=tolerancia+1;
    float gradient_i=tolerancia+1;
    float gradient_t=tolerancia+1;
    float weight=0;
    float threshold=0;
    float desired_output;
    float estimated_y [100];
    while(epsi_i<tolerancia && epochs< max_epochs)
      {
	for(int i=1; i< w.size(); i++)
	 {
	   estimated_y[i]=0;
