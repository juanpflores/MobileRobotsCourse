				       ...
				       sensor_msgs::LaserScan real_measurement)
{
    ...
    for (int i = 0; i < particle_measurements.size(); i++){
        ...
        for(int j = 1; j < particle_measurements[i].ranges.size(); j++){
            weights[i] = weights[i] + exp( -pow((real_measurement.ranges[j * 10] - particle_measurements[i].ranges[j]), 2) / SENSOR_NOISE);
        }
...
int main(int argc, char** argv)
{
    ...
    for(int i = 0; i < robots.size(); i++){
        robots[i].x = robots[i].rng->uniformReal(0, 10);
        robots[i].y = robots[i].rng->uniformReal(0, 10);
