...
int main(int argc, char** argv)
{
     ...
     */
    float grad_mag;
    float tolerance = 1500.0;
    int epochs = 0;

    std::vector<float> desired_output;
    desired_output.resize(training_labels.size());
    for(int i = 0; i < training_labels.size(); ++i){
        if(training_labels[i] == digit_to_train) {
            desired_output[i] = 1.0f;
        } else {
            desired_output[i] = 0.0f;
        }
    }

    while(abs(grad_mag) > tolerance && epochs < max_epochs) {
        grad_mag = 0;
        for(int i = 0; i < training_images.size(); ++i){
            ...
            for(int j = 0; j < training_images.size(); ++j){
                gradient_i += (estimated_y - desired_output[j])*(estimated_y*(1-estimated_y));
            }

            for(int j = 0; j < p.w.size(); j++){
                p.w[j] -= delta * gradient_i * (float)training_images[i].data[j];
                grad_mag += gradient_i * (float)training_images[i].data[j];
            }
