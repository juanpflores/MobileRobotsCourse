    ...
    for i in range(len(p)):
        for j in range(len(p[i])):
            ...
            if cell_if_undershoot >= len(p[i]): 
                cell_if_undershoot -= len(p[i])
            if cell_if_correct < 0:
                cell_if_correct += len(p[i])
            if cell_if_correct >= len(p[i]):
                cell_if_correct -= len(p[i])
            if cell_if_overshoot < 0:
...
def move_y(cells, p):
    ...
    for i in range(len(p)):
        for j in range(len(p[i])):
            ...
            if cell_if_undershoot >= len(p):
                cell_if_undershoot -= len(p)
            if cell_if_correct < 0:
                cell_if_correct += len(p)
            if cell_if_correct >= len(p):
                cell_if_correct -= len(p)
            if cell_if_overshoot < 0:
                cell_if_overshoot += len(p)
            if cell_if_overshoot >= len(p): 
                cell_if_overshoot -= len(p)
            
            #Teorema de Probabilidad Total
            #primer indice renglones, segundo indice columnas
...
cmd = '' 
print 'Sandoval_Penilla Practica07'
print 'World:'
for i in range(len(map)):
    print map[i]
