	...
	nav_msgs::OccupancyGrid& map, nav_msgs::Path& result)
{
	...
	while(!open_list.empty() && current_node->index != idx_goal)
	{
		...
		std::cout << "Distancia " << current_node->distance << std::endl;
		current_node->in_closed_list = true;
	...
	nav_msgs::OccupancyGrid& map, nav_msgs::Path& result)
{
	...
	*/
	while(!open_list.empty() && current_node->index != idx_goal)
	{
		...
		std::cout << "F_value " << current_node->f_value << std::endl;
		current_node->in_closed_list = true;
		...
		for(size_t i=0; i < node_neighbors.size(); i++)
		{
			...
			if(dist < neighbor->distance)
			{
				...
				delta_x = idx_goal % map.info.width - idx_start % map.info.width;
				delta_y = idx_goal / map.info.width - idx_start / map.info.width;
				man_dist = delta_x + delta_y;
				neighbor->f_value = dist + man_dist;
