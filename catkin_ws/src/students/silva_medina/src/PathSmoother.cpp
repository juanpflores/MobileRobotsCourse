...
nav_msgs::OccupancyGrid PathSmoother::GetNearnessMap(nav_msgs::OccupancyGrid& map, float nearness_radius)
{
    ...
    */
    ...
    int counter =  0;
   for(int i = -step; i<=step; i++) //Se calculan los valores y se le asigna los valores a las celdas
    ...
    }// Se verifica si no existen valores más altos en cuanto a proximidad a otras celdas
    return nearnessMap;
}
...
nav_msgs::Path PathSmoother::SmoothPath(nav_msgs::Path& path, float alpha, float beta)
{
    ...
    if(path.poses.size() < 3)
        return newPath;
    float tol =  0.00001 * path.poses.size();//Se asigna el valor de la tolerancia
    int pasos = 10000;//Numero de pasos maximos que realiza el algoritmo
    float grad_v = tol + 1;//Da un valor inicial al gradiente
    float delta  =  0.45;//Valor de delta
    
	while (grad_v >= tol && --pasos > 0){
        	grad_v = 0;
		//Calculo de los puntos iniciales
	...
	//Posicion inicial de la nueva ruta
        ...
        //Calculo del valor de gradiente
        grad_v += fabs(grad_x) + fabs(grad_y);
	
	for(int i = 1; i < path.poses.size() - 1; i++){
            //calculo de los puntos de 1 a k-1
            ...
            //calulo de la magnitud del gradiente 
            grad_v += fabs(grad_x) + fabs(grad_y);
        }
